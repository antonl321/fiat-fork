...
INTEGER(KIND=JPIM), SAVE :: IPROCW_SETUP=-1, IPROCV_SETUP=-1
CONTAINS
! ------------------------------------------------------------------
...
SUBROUTINE MPL_GROUPS_CREATE(KPROCW, KPROCV)
  ...
  USE MPI4TO8, ONLY : &
    ...
    MPI_CART_SUB => MPI_CART_SUB8, MPI_COMM_FREE => MPI_COMM_FREE8
#endif
...
IF(LGROUPSETUP) THEN
  ! Already set up: return early if the topology is the same, otherwise
  ! free the existing communicators so they can be recreated for the new
  ! processor distribution (e.g. redistribute.F90 calls SETUP_TRANS twice
  ! with different NPRTRW/NPRTRV after switching MPI communicators).
  IF(KPROCW == IPROCW_SETUP .AND. KPROCV == IPROCV_SETUP) RETURN
  CALL MPI_COMM_FREE(MPL_ALL_MS_COMM,   IERR)
  CALL MPI_COMM_FREE(MPL_ALL_LEVS_COMM, IERR)
  CALL MPI_COMM_FREE(MPL_COMM_GRID,     IERR)
  LGROUPSETUP = .FALSE.
ENDIF
